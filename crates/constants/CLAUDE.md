...
| `address_space` | S12. `REG = 1`, `RAM = 2`, `PC = 3`: nonzero, so no real memory tuple is all zeros. S21 added `DELEGATION_KECCAK_F = 4`, **one space per delegation family**: an anchor tuple must be unreachable from RAM and from any other family's, so a space is what separates them (`docs/spec/delegation.md` §3, §5). S22 added `DELEGATION_ECRECOVER = 5` and `DELEGATION_ECRECOVER_SCRATCH = 6` — the second is the one space that is neither memory nor an anchor: a recovery spans 2,048 rows and the value crossing between them rides the global multiset there, which is what this engine has instead of cross-row wiring (`docs/spec/ecrecover.md` §2.3). S23's tag is 7. |
...
| `secp256k1`, `ecrecover` | S22. The curve `family::ECRECOVER` recovers over, and that family's frame and row budget. Zero logic, as everywhere here: the limb tables were **computed** from `p = 2^256 − 2^32 − 977` and the generator rather than transcribed, and `crates/program/tests/secp256k1.rs` re-derives every one — including the fifteen `G_MULTIPLES`, which matters because the circuit spends them as **gate literals** rather than as setup columns (`docs/spec/ecrecover.md` §5.1). |
| `family` | S11. The append-only `FamilyId` table (0 add/sub/lui/auipc … 6 atomics, 7 `INIT_TEARDOWN`, since S14 RAM window 0 only; S14's 8 `ZERO_WINDOWS`; **S21's 9 `KECCAK_F`**, the first delegation family and the first family above the two window ones; **S22's 10 `ECRECOVER`**), `COUNT`, the height menu, the default heights, `DEFAULT_BYTECODE_SIZE_WORDS`, the decoded-table `CODE_VERSION`, and S20's `CYCLE_OWNING`: whether a family's rows are execution cycles, `true` for the seven instruction families and `false` for the two RAM window families **and for every delegation family**, append-only beside the ids. Only cycle-owning families' shards partition an execution in time (`docs/spec/block-proof.md` §4). **`HEIGHT_MENU` opens with `2^8` since S21** and `DEFAULT_HEIGHTS[KECCAK_F]` is that: a delegation family's rows are invocations, not halfwords, and `2^16` of them is 744 GB of forward pass (`docs/spec/delegation.md` §9). It is an even power because Mercury needs one. `DEFAULT_HEIGHTS[ECRECOVER]` is `2^20`, the other way round: its rows are *fractions* of an invocation — 2,048 to a recovery — so the height is what makes `RANGE16` and `TIMESTAMP` fit (`docs/spec/ecrecover.md` §6.1). |
