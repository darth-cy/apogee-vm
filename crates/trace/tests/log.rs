...
fn the_tags_are_the_frozen_constants() {
    ...
    assert_eq!(
        (
            ...
            address_space::ADVICE,
        ),
        (1, 2, 3, 4, 5, 6, 7)
    );
    // Every tag is nonzero, so no real tuple is all zeros, and 8 is the tag the
    // next delegation family takes — it names no space yet. S25b gave 7 to
    // `ADVICE`, which is why the delegation tags stop being contiguous; the
    // set below is what tells them apart, never a range
    // (`docs/spec/delegation.md` §3, `docs/spec/advice.md` §1).
    for tag in [0u8, 8, 255] {
        assert_eq!(AddressSpace::from_tag(tag), None, "tag {tag}");
    }
    // Advice is the one space nothing may write, and the one memory space
    // whose initial values are the prover's.
    assert!(!AddressSpace::Advice.writable());
    assert!(AddressSpace::Advice.chains());
    for space in [AddressSpace::Reg, AddressSpace::Ram, AddressSpace::Pc] {
        assert!(space.writable(), "{space:?}");
    }
