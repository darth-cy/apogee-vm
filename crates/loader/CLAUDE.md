...
| Path | What |
| --- | --- |
| `tests/vectors/{fib,echo,rvc-dense,amm,orderbook,vault,atomics,opcodes,heap,portability}.elf` | the committed guest ELFs; `atomics` (S11) is every A-extension instruction as the compiler emits it; `opcodes`, `heap` and `portability` are S12's |
