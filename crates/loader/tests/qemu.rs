...
//! too, and `crates/emulator/tests/qemu_outputs.rs` holds what it *computes* to
//! what QEMU computes — the exit status and fd 1, and nothing below that. This
//! file stays what it was: the guests'
...
//! guests, and `crates/emulator/tests/qemu_outputs.rs` compares the two
//! answers.
...
/// `crates/emulator/tests/qemu_outputs.rs`'s claim; this is the
...
/// `crates/emulator/tests/qemu_outputs.rs`'s claim, and the rows behind it are
/// `crates/checker/tests/jump_branch_slt.rs`'s.
#[test]
#[ignore = "needs a Linux host with qemu-user; run with --ignored"]
...
/// `crates/emulator/tests/qemu_outputs.rs`'s, which holds the two to the same
/// answer, and the rows behind it are `crates/checker`'s family suites'.
#[test]
#[ignore = "needs a Linux host with qemu-user; run with --ignored"]
...
/// which holds the two to the same answer, and the rows behind it are
/// `crates/checker/tests/mem_fill.rs`'s.
///
