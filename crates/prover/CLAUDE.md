...
- **The family-registration surface.** A family is proved when `constraints::family_circuit`
  ...
  **S-IO's three window families were three fills and two small arms** and, again, no change
...
- **`prove_block` is orchestration and nothing else** (S20): it refuses a `ShardPlan`
  ...
  cycles, so the plan counts 0 for each and their shards are the statement's: since S-IO
...
| File | Covers |
| --- | --- |
| `tests/common/mod.rs` | the S16 statement: `guests/addsub`'s committed ELF decoded with its family at `2^20` and everything else at `2^16`, traced into an archive, over a toy SRS whose `tau` is written down and whose archive is cached under `target/tmp` (`CARGO_TARGET_TMPDIR`), shared by the suites that include this module — `tests/acceptance.rs`, `tests/control.rs`, `tests/public_io.rs`, `crates/verifier/tests/cli.rs` and `crates/checker/tests/tamper.rs`; S17's, `guests/control`'s, with both of its execution families at `2^20` (`control_setup`, `control_archive`, `CONTROL_RESULT = 16`); S-IO's, `guests/public-io`'s (`public_io_setup`, `public_io_archive(program, advice)`, and `public_io_input` / `public_io_journal`, which are what the guest's own source says its input and its journal are for a given advice); `toy_tau` |
