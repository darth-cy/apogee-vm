...
- **The family-registration surface.** A family is proved when `constraints::family_circuit`
  ...
  **S25's three window families were three fills and two small arms** and, again, no change
  to the phase code. `family_fill` routes `PUBLIC_OUTPUT` to the **same `window` fill**
  `INIT_TEARDOWN` and `ZERO_WINDOWS` take, because its circuit is `ZERO_WINDOWS`' byte for
  byte and its init leaf is a literal 0 — there is no init column to fill, which is what
  stops a prover supplying the journal at timestamp 0. `PUBLIC_INPUT` and `ADVICE_WINDOWS`
  get one fill each, both over `trace::build_value_window_columns`: `fill::public_input`
  writes `M[2]` as `program::public_io_words(archive.io_streams().input)` — the one place a
  prover can put the bytes and have step 10c accept the proof — and refuses an input above
  `guest_memory::PUBLIC_PAYLOAD_BYTES` by name; `fill::advice` writes this window's slice of
  `archive.advice()` through `trace::advice_word`, and **nothing binds it**. The two arms
  are `shard_counts` and `window_of`: the first overrides the plan's zeroes with 1 for each
...
- **`prove_block` is orchestration and nothing else** (S20): it refuses a `ShardPlan`
  ...
  buffer, the last chunk padded by the column builders. Every **window** family runs no
  cycles, so the plan counts 0 for each and their shards are the statement's: since S25
  that is the two RAM window families, the two public value families and
  `ADVICE_WINDOWS`, all five overridden in `shard_counts`.
...
| File | Covers |
| --- | --- |
| `tests/common/mod.rs` | the S16 statement: `guests/addsub`'s committed ELF decoded with its family at `2^20` and everything else at `2^16`, traced into an archive, over a toy SRS whose `tau` is written down and whose archive is cached under `target/tmp` (`CARGO_TARGET_TMPDIR`), shared by the suites that include this module — `tests/acceptance.rs`, `tests/control.rs`, `tests/public_io.rs`, `crates/verifier/tests/cli.rs` and `crates/checker/tests/tamper.rs`; S17's, `guests/control`'s, with both of its execution families at `2^20` (`control_setup`, `control_archive`, `CONTROL_RESULT = 16`); S25's, `guests/public-io`'s (`public_io_setup`, `public_io_archive(program, advice)`, and `public_io_input` / `public_io_journal`, which are what the guest's own source says its input and its journal are for a given advice); `toy_tau` |
