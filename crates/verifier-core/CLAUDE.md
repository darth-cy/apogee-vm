...
- **`reduce_shard`**: the eleven steps of §6 that end at the opening claim, reachable
  since S20 in two public halves — `derive_global_phase` (steps 1–3 and the global
  transcript, once per statement) and `verify_shard_local` (steps 4–11, once per shard).
...
pub fn verify_shard_local(vk: &VerifyingKey, global: &GlobalChallenges, proof: &ShardProof,
                          public: &PublicInputs) -> Result<OpeningClaim, VerifyError>;  // 4-11
