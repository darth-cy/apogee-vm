...
| `src/gadgets.rs` (unit) | two range halfwords are the comparison's 32-bit word; the comparison returns two gates and eight lookups, each sign lookup the generic table's width; a `const` assertion holds `U16GetSign`'s keys above AND's |
| `src/jump_branch_slt.rs` (unit) | the legal masks are twelve distinct single bits |
