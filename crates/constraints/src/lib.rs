...
pub fn family_circuit(family: u32, trace_vars: u32) -> Option<FamilyCircuit> {
    ...
    let (artifact, channels) = match family {
        ...
        // height above does not name it. `ADVICE_WINDOWS` is held to
        // `INIT_TEARDOWN`'s height by `verifier_core::window_height`, not by
        // anything here (`docs/spec/advice.md` §5.0).
