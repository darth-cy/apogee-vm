---
title: 'S25 — Witness pipeline, real blocks, bench harness'
...
## Depends on / Inputs
- S24 supplies `guests/revm-block`, the `BlockWitness` format and the output-commitment structure. S10 supplies `io_digest`, the frozen public I/O digest convention that S24 consumes verbatim.
- S20's `prover`/`verifier` supply `BlockProof` and `ShardPlan` execution. S12's `TraceArchive` supplies the per-phase snapshots, and its execution phase supplies the execution timing.
- S10/S11 supply the guest build workflow and `ProgramIdentity`. S21 supply the delegation families, which this stage exercises but does not modify.

## Deliver
- The `host` crate gains `WitnessRecorder`. It pre-executes txs with native revm against an RPC-backed state database, records every touched account, storage slot, and code blob, and emits a `BlockWitness`. Add the host convenience wrappers `host::prove`/`host::verify` around S20's entry points. S20's `prove_block`/`verify_block` remain the main protocol entry points, and verifier signature discipline holds per this rule: the wrapper adds nothing to the verifier's inputs.
- **Mini-block mode** executes the first 1–2 txs of a *recent* mainnet block against a pseudo next-state. It claims no state-root recomputation, and the guest's output commitment covers per-tx results only.
- **Stateless full-block mode** (RSP-style) extends `BlockWitness` in the optional section S24 reserved. The extension carries the MPT nodes needed to authenticate pre-state against the parent state root and to recompute the post-state root in-guest. The guest asserts recomputed root == header state root and folds both roots into the output commitment. The fixture block is pinned by hash.
- `tools/bench` runs a proving job and emits a `BenchReport`. The report carries proving wall-clock, total guest cycles, shard count per family, and per-stage timings for execution, commit, GKR and opening, read from the `TraceArchive` phase sections. It adds peak memory if cheaply available and a $-cost estimate from a user-supplied $/GPU-hour figure. Its output mirrors what an ethproofs submission needs (block number/hash, proving time, cost, hardware description), as machine-readable JSON and a human table. Do this on the remote server. Only do mini-block mode locally but for stateless full-block mode, always use the remote dev server.
...
4. **Mini-block proof (the mini-block gate):** prove the mini-block fixture locally and get `verify` Ok. The public I/O digest recomputed host-side from the fixture matches the proof's public inputs.
