...
| `revm-block` | S24's guest, and the first with a crates.io dependency: [revm](https://github.com/bluealloy/revm) executing a block over a synthetic pre-state, `no_std` and `default-features = false`. Read it for three things a workload guest needs and the others do not. **A library plus a thin binary**, the `consistency` pattern of §2a, so the host can run the same source as the native oracle. **A hash hook**: `alloy-primitives`' `native-keccak` feature turns every `keccak256` in the image — revm's `KECCAK256` opcode, a contract's code hash, the guest's own commitments — into an `extern "C"` call the guest implements as `guest_sdk::keccak256`, which is how a dependency that has never heard of this VM ends up using its delegation. **One binary since S25a**: `revm-block` takes its witness on fd 0 and commits on fd 1, and both of those are provable ecalls now, bound by the public I/O digest it publishes at exit. `revm-block-embedded`, which carried the same witness in `.rodata` because neither was provable, is retired (`docs/handoff/S24-revm.md` §1). It is also the only guest with no committed ELF: 2.2 MB at `--release` is not a fixture worth keeping, and its suites build it from source |
