...
/// EIP-7002's withdrawal-request predeploy, and its real deployed bytecode,
/// read from mainnet with `eth_getCode`.
const WITHDRAWAL_REQUESTS: Address20 = [
    0x00, 0x00, 0x09, 0x61, 0xef, 0x48, 0x0e, 0xb5, 0x5e, 0x80, 0xd1, 0x9a, 0xd8, 0x35, 0x79, 0xa6,
    0x4c, 0x00, 0x70, 0x02,
];
const WITHDRAWAL_REQUESTS_CODE: &str = "";

/// EIP-7251's consolidation-request predeploy, likewise.
const CONSOLIDATION_REQUESTS: Address20 = [
    0x00, 0x00, 0xbb, 0xdd, 0xc7, 0xce, 0x48, 0x86, 0x42, 0xfb, 0x57, 0x9f, 0x8b, 0x00, 0xf3, 0xa5,
    0x90, 0x00, 0x72, 0x51,
];
const CONSOLIDATION_REQUESTS_CODE: &str = "";

/// The queue bookkeeping both request predeploys keep, and the values this
/// fixture starts them at: the excess counter, the per-block count, and the
/// queue's head and tail. Their layouts agree, which is why one list serves
/// both.
///
/// **The excess counter starts at 5 and not at 0**, and that is the whole point
/// of the fixture. Each call recomputes the excess as `previous + count -
/// target` when that is positive and 0 otherwise, so from zero with an empty
/// queue the call writes 0 over 0, changes nothing, and a post-state root
/// computed without the call at all would be identical -- the fixture would
/// accept the code being deleted again. From 5 it writes a smaller number, so
/// the root moves if and only if the call was made.
/// `the_prague_request_calls_move_the_root` is that check.
///
/// The queue bodies live from slot 4 up and are untouched: the queues are empty,
/// which takes the reset path rather than the dequeue path. An `SSTORE` reads
/// the original value first, so every slot written is also a slot read, and the
/// strict database refuses a slot the witness does not carry -- which is why
/// these are explicit rather than discovered at run time.
const REQUEST_QUEUE_SLOTS: [(u128, u128); 4] = [
    // (slot, starting value): excess, count, queue head, queue tail.
    (0, 5),
    (1, 0),
    (2, 0),
    (3, 0),
...
pub fn synthetic_stateless() -> (BlockWitness, Word32) {
    ...
    // The two Prague post-block request predeploys. Both queues are empty, so
    // each call takes the reset path: it reads the head and the tail, finds
    // them equal, zeroes them, updates the excess counter and writes a zero
    // count.
    for (address, code) in [
        (WITHDRAWAL_REQUESTS, WITHDRAWAL_REQUESTS_CODE),
        (CONSOLIDATION_REQUESTS, CONSOLIDATION_REQUESTS_CODE),
    ] {
        witness.accounts.push(AccountWitness {
            address,
            nonce: 1,
            balance: [0u8; 32],
            code: unhex(code),
            slots: REQUEST_QUEUE_SLOTS
                .iter()
                .map(|(slot, value)| (word(*slot), word(*value)))
                .collect(),
        });
    }
