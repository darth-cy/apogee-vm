...
impl FieldElement10x26 {
    ...
    ///
    /// apogee-vm: on the guest target this is the `MOD_MUL` delegation, and the
    /// result is *fully* normalized, which is stronger than the contract above.
    pub fn mul(&self, rhs: &Self) -> Self {
        #[cfg(target_arch = "riscv32")]
        let out = apogee::mul_mod_p(self, rhs);
        #[cfg(not(target_arch = "riscv32"))]
        let out = self.mul_inner(rhs);
        out
    }
    ...
    /// The magnitudes of arguments should be <= 8.
    ///
    /// apogee-vm: as [`Self::mul`]. The delegation has no squaring frame — one
    /// circuit serves both, with `a` and `b` the same value.
    pub fn square(&self) -> Self {
        #[cfg(target_arch = "riscv32")]
        let out = apogee::mul_mod_p(self, self);
        #[cfg(not(target_arch = "riscv32"))]
        let out = self.mul_inner(self);
        out
    }
}
...
mod apogee {
    ...
    /// A **[`packable`]** element's ten 26-bit limbs as eight 32-bit ones.
    ///
    /// Limb `i` holds bits `[26i, 26i + 26)` of the value and word `j` holds
    /// bits `[32j, 32j + 32)`, so each word reads one or two limbs. What
    /// `packable` is required for is the top word alone: `l[9] < 2^22` is what
    /// makes `l[9] << 10` lose nothing.
    fn pack(x: &FieldElement10x26) -> [u32; 8] {
        let l = &x.0;
        [
            l[0] | (l[1] << 26),
            (l[1] >> 6) | (l[2] << 20),
    ...
    ///
    /// `false` from the shim is exactly `-ENOSYS` — an executor with no circuit
    /// for this family, which is every executor but this VM's — and then the
    /// answer comes from this file's own `mul_inner`. So the two paths are one
    /// function's two implementations and agree on the value; they do **not**
    /// agree on the representation, the delegated one being canonical where
    /// `mul_inner`'s is only weakly normalized. Nothing in this crate compares
